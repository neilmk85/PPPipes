...
import { Calendar, ChevronDown, Wind, Download } from 'lucide-react'
        ...
        <div className="absolute right-0 top-full mt-2 z-50 bg-white rounded-2xl shadow-xl border border-gray-100 p-3 w-60">
            ...
            {PRESETS.map(p => (
              <button key={p.label} onClick={() => { onChange(p.from(), p.to()); setOpen(false) }}
                className={`w-full text-left px-3 py-2 text-sm rounded-xl font-medium transition-colors ${
                  from === p.from() && to === p.to() ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-50'
                }`}>{p.label}</button>
            ))}
          </div>
          <div className="border-t border-gray-100 pt-3 space-y-2">
            ...
            <input type="date" value={tmpFrom} onChange={e => setTmpFrom(e.target.value)}
              className="w-full px-3 py-1.5 text-xs border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-400 text-gray-700" />
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { onChange(tmpFrom, tmpTo); setOpen(false) }} disabled={!tmpFrom || !tmpTo}
                className="flex-1 py-1.5 text-xs font-bold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 disabled:opacity-40">Apply</button>
            </div>
          </div>
        </div>
      )}
...
function exportCSV(rows: any[], from: string, to: string, contractorName: string) {
  ...
  const blob = new Blob([`Winding 2 Report\nContractor: ${contractorName}\nPeriod: ${from||'All'} to ${to||'All'}\n\n${csv}`], { type: 'text/csv' })
  const a = document.createElement('a'); a.href = URL.createObjectURL(blob)
  a.download = `winding2-report-${from||'all'}-to-${to||'all'}.csv`; a.click()
  URL.revokeObjectURL(a.href)
}

export default function Winding2ReportPage() {
  const [from, setFrom] = useState('')
  const [to,   setTo]   = useState('')

  const { data, isLoading } = useQuery({
    queryKey: ['winding2-report', from, to],
  ...
  const contractor = (assignments as any[]).find((a: any) => a.processType === 'WINDING_2')
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Hero */}
      <div className="relative overflow-hidden bg-gradient-to-br from-indigo-700 via-indigo-600 to-violet-600 px-8 pt-7 pb-6 shadow-[0_8px_40px_rgba(79,70,229,0.30)]">
        ...
        <div className="relative flex items-start justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-semibold text-indigo-200 uppercase tracking-widest mb-0.5">Production · Stage Report</p>
              <h1 className="text-2xl font-extrabold text-white tracking-tight">Winding 2 Report</h1>
              <p className="text-sm text-indigo-200 mt-0.5">{contractorName}</p>
            </div>
          </div>
        ...
        <div className="relative mt-5 flex items-stretch gap-px bg-white/10 rounded-xl overflow-hidden border border-white/10">
          {[
            ...
            <div key={i} className="flex-1 px-5 py-3 bg-white/5">
              <p className="text-[10px] font-bold text-indigo-200 uppercase tracking-widest mb-0.5">{s.label}</p>
      ...
      <div className="p-6">
        <div className="bg-white rounded-2xl shadow-sm ring-1 ring-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100">
            <h2 className="text-sm font-bold text-gray-700 uppercase tracking-wide">Winding 2 Entries</h2>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-gray-400">
              <Wind size={36} className="mb-3 opacity-30" />
              <p className="font-semibold text-gray-500">No Winding 2 entries found</p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-indigo-50 border-b border-indigo-100">
                    <th className="px-4 py-3 text-left text-[11px] font-bold text-indigo-700 uppercase tracking-widest">Date</th>
                    <th className="px-4 py-3 text-left text-[11px] font-bold text-indigo-700 uppercase tracking-widest">PO Number</th>
                    <th className="px-4 py-3 text-left text-[11px] font-bold text-indigo-700 uppercase tracking-widest">Pipe Config</th>
                    <th className="px-4 py-3 text-right text-[11px] font-bold text-indigo-700 uppercase tracking-widest">Dia (mm)</th>
                    <th className="px-4 py-3 text-right text-[11px] font-bold text-indigo-700 uppercase tracking-widest">Processed</th>
                    <th className="px-4 py-3 text-right text-[11px] font-bold text-indigo-700 uppercase tracking-widest">Completed</th>
                    <th className="px-4 py-3 text-right text-[11px] font-bold text-indigo-700 uppercase tracking-widest">Rejected</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {rows.map((r: any, i: number) => (
                    <tr key={i} className="hover:bg-indigo-50/40 transition-colors">
                      ...
                      <td className="px-4 py-3 font-mono text-xs text-violet-700 font-bold">{r.productionOrder?.poNumber ?? '—'}</td>
                      ...
                      <td className="px-4 py-3 text-right font-bold text-indigo-700">{Number(r.pipesCompleted).toLocaleString()}</td>
                ...
                <tfoot>
                  <tr className="bg-indigo-50 border-t border-indigo-100">
                    ...
                    <td className="px-4 py-3 text-right text-base font-extrabold text-indigo-700">{totalPipes.toLocaleString()}</td>
                    <td />
                  </tr>
                </tfoot>
              </table>
