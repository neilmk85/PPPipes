          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
              {RATE_FIELDS.map(f => (
                <div
                  key={f.key}
                  className="bg-white rounded-2xl shadow-md ring-1 ring-gray-100 p-4 flex flex-col gap-2"
                >
                  <div>
                    <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-widest leading-none">{f.label}</p>
                    <p className="text-lg font-extrabold text-gray-900 tabular-nums mt-1 leading-none">
                      {fmt(cfg?.[f.key] ?? '')}
                    </p>
